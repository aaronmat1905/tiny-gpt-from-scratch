...
def build_itos(vocab):
    ...
    return {i:vocab[i] for i in range(len(vocab))}
...
def decode_ids(ids, itos):
    ...
    return "".join(decode_int(tok, itos) for tok in ids)
...
def softmax_overflow_demo(large_value):
    ...
    """
    # Put the value into a 1-length array 
    ...
    # Check whether it overflowed
    overflowed = not np.isfinite(naive_exp)
    return {
        "naive_exp":naive_exp, 
...
def read_text_file(text_blob):
    ...
    if not isinstance(text_blob, str): 
        raise TypeError("text_blob not a string")
    if text_blob == "": 
...
def vectorize_counts_add_at(vocab_size, data):
    ...
    current = data[:-1]
    nxt = data[1:]
    np.add.at(counts, (current, nxt), 1)
    return counts
...
def log_prob_of_pair(p_matrix, current_id, next_id):
    ...
    prob = p_matrix[current_id][next_id]
    return np.log(prob)
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    encoded = make_2d_zeros(len(ids), vocab_size)
